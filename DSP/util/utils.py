...
import logging
from util.dist_util import is_main_process, get_rank
...
def summary_writer(args, log_dir=None, filename_suffix=''):
    ...
    """
    if log_dir is None:
        args.log_dir = os.path.join(args.save_dir, "{}_bs_{}".format(args.backbone, args.ssl_batchsize),
                                    ctime().replace(' ', '_'))
        mkdir(args.log_dir)
    else:
...
def evalExp(gtBin, cur_prob, thres, validMap=None, validArea=None):
    ...
    if np.any(validMap) != None:
        ...
        # print('fnarray',np.unique(fnArray,return_counts=True))
    else:
...
def eval_image_rewrite(gt_image, prob, cl_index):
    ...
    # print(cl_gt.shape, np.unique(cl_gt,return_counts=True))
    # print(valid_gt.shape, np.unique(valid_gt,return_counts=True))
