...
class JSD(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.dense= args.dense_cl
    def forward(self, net_1_logits, net_2_logits):
        if self.dense==True:
            net_1_logits = net_1_logits.view(net_1_logits.size(0), -1)
            net_2_logits = net_2_logits.view(net_2_logits.size(0), -1)


        net_1_probs = F.softmax(net_1_logits+ 1e-10, dim=1)
        net_2_probs = F.softmax(net_2_logits+ 1e-10, dim=1)

        total_m = 0.5 * (net_1_probs + net_2_probs)

        return 0.5 * (F.kl_div(F.log_softmax(net_1_logits, dim=1), total_m, reduction="batchmean") +
                            F.kl_div(F.log_softmax(net_2_logits, dim=1), total_m, reduction="batchmean"))
...
def similarity_preserving_loss_cd(A_t, A_s, A_t1, A_s1 ):
    ...
    Q_t = A_t.reshape([b1, c1 * h1 * w1])
    Q_s = A_s.reshape([b2, c2 * h2 * w2])
    Q_t1 = A_t1.reshape([b1, c1 * h1 * w1])
    Q_s1 = A_s1.reshape([b2, c2 * h2 * w2])
    # evaluate normalized similarity matrices (eq 3)
    G_t = torch.mm(Q_t, Q_s.t())
    # G_t = G_t / G_t.norm(p=2)
    G_t = torch.nn.functional.normalize(G_t)

    G_s = torch.mm(Q_t1, Q_s1.t())
    # G_s = G_s / G_s.norm(p=2)
    G_s = torch.nn.functional.normalize(G_s)
